...
function OpenBackAndBorderWindow()
	local border_buf = vim.api.nvim_create_buf(false, true)
	local border_buf_name = systemVar.todoPath .. "border_buf"
	...
	for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
--		print("bufnr : " .. bufnr .. vim.api.nvim_buf_get_name(bufnr))
		if vim.api.nvim_buf_get_name(bufnr) == border_buf_name then
			existing_buf = bufnr
			break
		end
	end
	...
	if existing_buf == -1 then
		vim.api.nvim_buf_set_name(content_buf, border_buf_name)
		vim.api.nvim_buf_call(content_buf, function()
			vim.api.nvim_command("edit " .. border_buf_name)
		end)
	end
...
function OpenFloatingWindow()
	local syntax_path = "~/.config/nvim/lua/vkdev/plugins_config/TodoList/weeks.lua"
--	local syntax_path = "~/.config/nvim/lua/vkdev/plugins_config/todo_plug/.lua"
	local file_path = systemVar.weekTodoPath
	local next_path = systemVar.nextTodoPath
...
--		print("bufnr : " .. bufnr .. vim.api.nvim_buf_get_name(bufnr))
	...
	vim.api.nvim_command("edit " .. systemVar.nextTodoPath)
...
vim.cmd[[ command! -nargs=0 TodoWindow :lua OpenFloatingWindow()]]
--vim.cmd[[ command! -nargs=0 TodoWindowTest :lua OpenBackAndBorderWindow()]]
